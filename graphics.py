...
outline-color, fill-color and LineG-width. Graphical objects also
...
#     Made Polygon accept a list of PointGs in constructor
...
class GraphWin(tk.Canvas):
    ...
    def getMouse(self):
        """Wait for mouse click and return PointG object representing
        the click"""
    ...
    def _onClick(self, e):
        ...
        if self._mouseCallback:
            self._mouseCallback(PointG(e.x, e.y))
...
class PointG(GraphicsObject):
    ...
    def __repr__(self):
        return "PointG({}, {})".format(self.x, self.y)
...
class _BBox(GraphicsObject):
    ...
    # (opposite corners) LineG segment is a degenerate case.
    ...
    def getCenter(self):
        p1 = self.p1
        p2 = self.p2
        return PointG((p1.x+p2.x)/2.0, (p1.y+p2.y)/2.0)
...
class Circle(Oval):
    ...
    def __init__(self, center, radius):
        p1 = PointG(center.x-radius, center.y-radius)
        p2 = PointG(center.x+radius, center.y+radius)
...
class LineG(_BBox):
    ...
    def __repr__(self):
        return "LineG({}, {})".format(str(self.p1), str(self.p2))

    def clone(self):
        other = LineG(self.p1, self.p2)
        other.config = self.config.copy()
        return other
...
class Polygon(GraphicsObject):
    ...
    def __init__(self, *PointGs):
        # if PointGs passed as a list, extract it
        if len(PointGs) == 1 and type(PointGs[0]) == type([]):
            PointGs = PointGs[0]
        self.PointGs = list(map(PointG.clone, PointGs))
        GraphicsObject.__init__(self, ["outline", "width", "fill"])

    def __repr__(self):
        return "Polygon"+str(tuple(p for p in self.PointGs))
        
    def clone(self):
        other = Polygon(*self.PointGs)
        other.config = self.config.copy()
        return other

    def getPointGs(self):
        return list(map(PointG.clone, self.PointGs))

    def _move(self, dx, dy):
        for p in self.PointGs:
            p.move(dx,dy)
...
def test():
    ...
    t = Text(PointG(5,5), "Centered Text")
    t.draw(win)
    p = Polygon(PointG(1,1), PointG(5,3), PointG(2,7))
    p.draw(win)
    e = Entry(PointG(5,6), 10)
    e.draw(win)
    win.getMouse()
    p.setFill("red")
    p.setOutline("blue")
